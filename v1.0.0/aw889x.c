...
//#define AW889X_I2C_NAME "i2c_smartpa"

#define AW889X_VERSION "v1.0.0"
...
static char *aw889x_fw_name = "aw889x_fw.bin";
...
static void aw889x_init(struct aw889x *aw889x)
{
	...
	reg_val |= AW889X_BIT_I2SCTRL_CHS_MONO | 
			...
			AW889X_BIT_I2SCTRL_FMS_20BIT |
			AW889X_BIT_I2SCTRL_BCK_64FS |
...
static int aw889x_load_fw(struct aw889x *aw889x)
{
	...
	return request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
								aw889x_fw_name, aw889x->dev, GFP_KERNEL,
...
static void aw889x_smartpa_cfg(struct aw889x *aw889x, bool flag)
{
	pr_info("%s, flag = %d\n", __func__, flag);
...
static int aw889x_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
        ...
        aw889x_smartpa_cfg(aw889x, false);
		...
		/* Start DSP */
		mutex_lock(&aw889x->dsp_lock);
        aw889x_smartpa_cfg(aw889x, true);
		mutex_unlock(&aw889x->dsp_lock);
	}
...
static int aw889x_probe(struct snd_soc_codec *codec)
{
	...
	if (codec->dev->of_node)
		dev_set_name(codec->dev, "%s", "aw889x_smartpa");
